...
const stripe = require('stripe')(process.env.STRIPE_KEY || 'your_stripe_key');
...
exports.chargeCustomer = app.post('/charge', function chargeCustomer(req, res) {
  ...
  stripe.charges.create(stripeRequestData, function(err, charge) {
    if (err) {
...
app.listen(port, () =>
  console.log(`Stripe Charge Customer GCF server listening on port ${port}!`),
);
